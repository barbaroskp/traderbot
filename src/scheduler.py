...
Cycle (every scan_interval_minutes):
  ...
  4. Generate signals (6-indicator confluence)
  5. Execute signals (simple sizing: base + high conviction + daily loss)
...
class Scheduler:
    ...
    async def start(self) -> None:
        ...
        # Write initial risk state so dashboard shows correct state after restart
    ...
    async def _run_cycle(self) -> None:
        ...
        if tradeable and not signals:
            log.warning(
                ...
                extra={"tradeable": len(tradeable), "risk_state": risk_state.value},
            )
        ...
        if self.risk.daily_kill_active:
            ...
            if remaining_open:
                log.warning(
                    ...
                    extra={"count": len(remaining_open), "daily_pnl": round(self.risk.daily_pnl, 4)},
                )
        ...
        # ── 7. Execute signals (best first by score) ────────────
        signals_sorted = sorted(
            signals, key=lambda s: (s.confluence_score, s.weighted_score), reverse=True
        )
        ...
        if daily_blocked:
            log.warning(
                "daily loss circuit breaker: entries paused",
                extra={
            ...
            for sig in signals_sorted:
                ...
                # ── Leverage: base or high conviction ────────────
                is_high_conviction = (
                ...
                final_leverage = self.cfg.leverage
                if is_high_conviction:
                    final_leverage = self.cfg.leverage_high_conviction
                final_leverage = min(final_leverage, self.cfg.max_leverage_allowed)

                # ── Position size (Kelly → ATR → Fraction) ──────
                ...
                # High conviction → larger margin allocation
                if is_high_conviction:
                    ...
                    log.info(
                        ...
                        extra={
                            ...
                            "score": round(sig.weighted_score, 1),
                            "leverage": final_leverage,
                        },
                    )
                ...
                sl_bps, tp_bps = _compute_tp_sl_bps(self.cfg, snap, snap.mid_price)
                ...
                if net_tp_bps <= 0 or (net_sl_bps > 0 and net_tp_bps / net_sl_bps < 1.5):
                    log.warning(
                        "fee guard: skipping low-expectancy trade",
                        extra={
                            "symbol": sig.symbol,
                ...
                result = await self.execution.execute_signal(
                    ...
                    qty=qty,
                    current_total_margin=current_margin,
                    leverage_override=final_leverage if is_high_conviction else None,
                )
                if result:
                    log.info(
                        "execution result",
    ...
    async def _sync_live_balance(self) -> None:
        ...
        try:
            ...
            if balance <= 0:
                log.warning("live balance sync skipped: non-positive", extra={"balance": balance})
                return
    ...
    def _extract_live_balance(raw: Any) -> float:
        ...
        priority_keys = (
            "balance", "equity", "walletBalance", "marginBalance",
            "accountEquity", "availableBalance", "availableMargin",
        )
    ...
    async def _reconcile_live_state(self) -> None:
        ...
        try:
            ...
            log.warning("live reconcile failed", extra={"error": str(exc)})
            return
        ...
        # DB open but no exchange position → close stale DB row
        ...
        # Exchange position but no DB row → create placeholder
