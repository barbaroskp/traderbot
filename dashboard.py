...
async def dashboard():
    ...
    print("  HESAP")
    print("  " + "-" * 56)
    print(f"  Baslangic:     {initial_capital:>8.2f} USDT")
    print(f"  Bakiye:        {balance:>8.2f} USDT")
    print(f"  Equity:        {equity:>8.2f} USDT  ({equity_change:+.2f})")
    print(f"  Margin:        {used:>8.2f} USDT")
    print(f"  Kullanilabilir:{available:>8.2f} USDT")
    print(f"  Acik PnL:      {unrealised:>+.4f} USDT")
    print("  " + "-" * 56)
    print()
    print("  PERFORMANS")
    print("  " + "-" * 56)
    print(f"  Realize PnL:   {realised_pnl:>+.4f} USDT")
    print(f"  Acik PnL:      {total_live_pnl:>+.4f} USDT")
    print(f"  TOPLAM PnL:    {total_pnl:>+.4f} USDT")
    ...
    print("  " + "-" * 56)
    print(f"  Toplam islem:  {total_closed}")
    print(f"  Kazanan:       {wins}  ({winrate:.1f}%)")
    print(f"  Kaybeden:      {losses}")
    print(f"  Ort. kazanc:   {avg_win:>+.4f} USDT")
    print(f"  Ort. kayip:    {avg_loss:>+.4f} USDT")
    ...
    print(f"  Long:          {long_count} islem   PnL: {long_pnl:>+.4f} USDT")
    print(f"  Short:         {short_count} islem   PnL: {short_pnl:>+.4f} USDT")
    print("  " + "-" * 56)
    ...
    print(f"  Risk:          {risk_icon.get(current_risk, current_risk)}")
    print(f"  Sebep:         {risk_reason}")
    status = (_run.stdout or "").strip()
    running = status in ("active", "activating")
    print(f"  Servis:        {'CALISIYOR' if running else 'DURDU'}")
    print(f"  Scan:          {cfg.scan_interval_minutes} dk")
    print(f"  Max pos:       {cfg.max_open_positions}")
    print(f"  Leverage:      {cfg.leverage}x")
    print(f"  TP:            {cfg.tp_bps} bps")
    print(f"  SL:            {cfg.sl_bps} bps")
    print(f"  DB:            {_db_abs}")
    print("  " + "-" * 56)
    print()
    print("=" * W)
